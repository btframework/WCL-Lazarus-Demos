...
type
  TfmMain = class(TForm)
    ...
    btGetScanParams: TButton;
    laScanInterval: TLabel;
    edScanInterval: TEdit;
    laScanWindow: TLabel;
    edScanWindow: TEdit;
    btSetScanParams: TButton;
    btGetAdvParams: TButton;
    laAdvInterval: TLabel;
    edAdvInterval: TEdit;
    btSetAdvParams: TButton;
    cbIBeacon: TCheckBox;
    ...
    procedure btGetScanParamsClick(Sender: TObject);
    procedure btSetScanParamsClick(Sender: TObject);
...
procedure TfmMain.btGetScanParamsClick(Sender: TObject);
var
  Radio: TwclBluetoothRadio;
  Res: Integer;
  Interval: Word;
...
begin
  Radio := GetRadio;
  if Radio <> nil then begin
    Res := Radio.GetLeScanParams(Interval, Window);
    if Res = WCL_E_SUCCESS then begin
      edScanInterval.Text := IntToStr(Interval);
      edScanWindow.Text := IntToStr(Window);
    end else
      MessageDlg('Error: 0x' + IntToHex(Res, 8), mtError, [mbOK], 0);
  end;
end;

procedure TfmMain.btSetScanParamsClick(Sender: TObject);
var
  Radio: TwclBluetoothRadio;
  Res: Integer;
begin
  Radio := GetRadio;
  if Radio <> nil then begin
    Res := Radio.SetLeScanParams(StrToInt(edScanInterval.Text),
      StrToInt(edScanWindow.Text));
    if Res <> WCL_E_SUCCESS then
      MessageDlg('Error: 0x' + IntToHex(Res, 8), mtError, [mbOK], 0)
    else
      ShowMessage('Scan parameters have been changed. Restart Beacon Watcher');
  end;
end;

procedure TfmMain.btGetAdvParamsClick(Sender: TObject);
var
  Radio: TwclBluetoothRadio;
  Res: Integer;
  Interval: Word;
begin
  Radio := GetRadio;
  if Radio <> nil then begin
    Res := Radio.GetLeAdvParams(Interval);
    if Res = WCL_E_SUCCESS then
      edAdvInterval.Text := IntToStr(Interval)
    else
      MessageDlg('Error: 0x' + IntToHex(Res, 8), mtError, [mbOK], 0);
  end;
end;

procedure TfmMain.btSetAdvParamsClick(Sender: TObject);
var
  Radio: TwclBluetoothRadio;
  Res: Integer;
begin
  Radio := GetRadio;
  if Radio <> nil then begin
    Res := Radio.SetLeAdvParams(StrToInt(edAdvInterval.Text));
    if Res <> WCL_E_SUCCESS then
      MessageDlg('Error: 0x' + IntToHex(Res, 8), mtError, [mbOK], 0)
    else
      ShowMessage('Advertisement parameters have been changed. Restart Advertiser');
  end;
end;
...
procedure TfmMain.wclBluetoothLeBeaconWatcherAdvertisementReceived(
  Sender: TObject; const Address: Int64; const Timestamp: Int64;
  const Rssi: Shortint; const Data: TwclBluetoothLeAdvertisementFrameRawData);
begin
